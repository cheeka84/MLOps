...
output_dir = "/content/tourism_package_pred"
rf_model_outpath = os.path.join(output_dir, "rf_pipeline.joblib")
xgb_model_outpath = os.path.join(output_dir, "xgb_pipeline.joblib")
best_model_outpath = os.path.join(output_dir, "best_pipeline.joblib")
...
# ---- shared preprocessing (imputer+scaler for num, imputer+OHE for cat) ----
...
with mlflow.start_run(run_name="rf_vs_xgb"):
    ...
    with mlflow.start_run(run_name="rf", nested=True):
        rf_gs = GridSearchCV(
            ...
            scoring=auc_scorer, error_score="raise"
        )
        ...
        rep_tr = classification_report(ytrain, yhat_tr, output_dict=True, zero_division=0)
        ...
        mlflow.log_artifact(rf_model_outpath, artifact_path="models")
        print("saved rf to:", rf_model_outpath)

    # ======================= xgboost =======================
    # compute simple imbalance weight from ytrain
    ...
    with mlflow.start_run(run_name="xgb", nested=True):
        xgb_gs = GridSearchCV(
            xgb_pipe, xgb_grid, cv=cv, n_jobs=-1,
            scoring=auc_scorer, error_score="raise"
        )
        ...
        mlflow.log_artifact(xgb_model_outpath, artifact_path="models")
        print("saved xgb to:", xgb_model_outpath)

    # ======================= choose best & upload =======================
    # re-use the last computed metrics from the two nested runs by recomputing here quickly:
    ...
    if abs(rf_auc - xgb_auc) < 1e-6:
        # tie-breaker by f1 at the threshold
    ...
    mlflow.log_artifact(best_model_outpath, artifact_path="models")
    ...
    api.upload_file(
        path_or_fileobj=best_model_outpath,
